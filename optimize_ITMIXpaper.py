...
def params(parser):
    ...
    type=str2bool,
    default=False,
    help="If True, compute regularizatgion as an reduce_mean and not a sum"
)
...
def cost_vol(params,state):
    ...
    cost = 0.5 * tf.reduce_mean(
            ( (state.volumes[ACT] - ModVols[ACT]) / params.opti_vol_std )** 2
    )
    #print(cost)
...
#    if not hasattr(state, "flowdirx"):
    ...
    if params.fix_opti_normalization_issue:
        REGU_S = (params.opti_regu_param_slidingco) * (tf.math.reduce_mean(dadx**2) + tf.math.reduce_mean(dady**2)) + 10**10 * tf.math.reduce_mean( tf.where(state.slidingco >= 0, 0.0, state.slidingco**2) ) 
    else: 
        REGU_S = (params.opti_regu_param_slidingco) * (tf.nn.l2_loss(dadx) + tf.nn.l2_loss(dady))+ 10**10 * tf.math.reduce_mean( tf.where(state.slidingco >= 0, 0.0, state.slidingco**2) ) 
...
def skip_infer_params(state, params):
    ...
    if params.opti_vol_factor_path is not None:
        vol = tf.reduce_sum(tf.where(state.icemaskobs > 0.5, state.thkinit,state.icemaskobs))*(state.dx**2)/(10**9)
        state.volume_weights = tf.where(state.icemaskobs > 0.5, 1.0, state.volume_weights)
        state.volumes = tf.where(state.icemaskobs > 0.5,vol, state.volumes)       
        print('Area (km^2) : ',np.round(TotalArea.numpy(),2),' and volume (km^3) : ',np.round(vol,2))      
